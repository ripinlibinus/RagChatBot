...
json_convertion_prompt = ChatPromptTemplate.from_messages([
    (
        ...
        "14. is_hard_filter = isi angka 1 jika filter berisi salah satu dari harga_min, harga_max, kamar_tidur, luas_bangunan, luas_tanah. jika tidak isi dengan angka 0 (wajib ada)\n"
...
def fetch_property(x):
    ...
    print(f"[italic bold green]{'Menemukan data properti...' if filter_result != None and filter_result.text != '' else 'Tidak menemukan data properti'}[/italic bold green]\n")
    print("[italic bold green]Menyiapkan jawaban kepada user ... [/italic bold green]\n")

    # list = doc_to_json(filter_result.text)
    # print(list)
    report_param["doc"] = filter_result.text
...
property_finder_prompt = ChatPromptTemplate.from_messages([
    ...
    (
        ...
        "Berikan jawaban yang ringkas, tepat dan padat. \n"
...
if __name__ == "__main__":
    ...
    ans = build_chain({
        "session_id": "1234",
        "question": "cari rumah yang komplek nya ada fasilitas lapangan basket",
        "user_name": "Tester"
    })
    print(ans)
