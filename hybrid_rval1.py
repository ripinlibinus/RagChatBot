...
# JSON conversion untuk SQL filter (paginate=5)
...
def fetch_sql(x):
    """Panggil API SQL dengan paginate=5 + dukung pagination lanjutan bila param sama."""
...
# ====== SELECTORS & PREDICATES (SQL-first logic) ======
def select_top5_sql_only(x):
    """Ambil Top-5 dari SQL saja (tanpa Vector)."""
    sql_text = x["sql_text"]
    sql_blocks = dedupe_by_link_or_head(split_sql_blocks(sql_text))
    selected = list(islice(sql_blocks, 5))
    fused_text = "\n\n".join(selected)

    report_param["doc"] = fused_text
    report_param["sql_count"] = len(sql_blocks)
    report_param["vector_count"] = 0
    report_param["selected_count"] = len(selected)
    print(f"[bold green]Top-5 (SQL only): {len(selected)} item[/bold green]\n")
    return fused_text

def select_top5_vector_only(x):
    """Ambil Top-5 dari Vector (dipakai saat SQL kosong)."""
    vec_items = dedupe_by_link_or_head(x["vec_items"])
    selected = list(islice(vec_items, 5))
    fused_text = "\n\n".join(selected)

    report_param["doc"] = fused_text
    report_param["sql_count"] = 0
    report_param["vector_count"] = len(vec_items)
    report_param["selected_count"] = len(selected)
    print(f"[bold green]Top-5 (Vector only): {len(selected)} item[/bold green]\n")
    return fused_text

def has_sql_data(x):
    """Predicate untuk branch: True jika SQL mengembalikan minimal 1 blok listing."""
    return len(split_sql_blocks(x.get("sql_text",""))) > 0
...
# --- Stage 1: siapkan rewrite + history dkk
...
# --- Stage 3: siapkan SQL dulu (tanpa Vector)
fetch_sql_chain = json_convertion_chain | RunnableLambda(fetch_sql)
sql_stage = RunnableParallel(
    rewrite_question = itemgetter("rewrite_question"),
    question         = itemgetter("question"),
    session_id       = itemgetter("session_id"),
    sql_text         = fetch_sql_chain,   # HANYA SQL di tahap ini
)

# --- Stage 4: Branch — jika SQL ada data ⇒ SQL-only; jika kosong ⇒ ambil Vector lalu Vector-only
def property_answer_from_sql_only():
    return RunnableParallel(
        question      = itemgetter("question"),
        data_property = RunnableLambda(select_top5_sql_only),
    ) | property_finder_prompt | llm | StrOutputParser()

def property_answer_from_vector_only():
    return (
        RunnableParallel(
            rewrite_question = itemgetter("rewrite_question"),
            question         = itemgetter("question"),
            session_id       = itemgetter("session_id"),
            vec_items        = RunnableLambda(fetch_vector)   # baru panggil Vector DI SINI
        )
        | RunnableParallel(
            question      = itemgetter("question"),
            data_property = RunnableLambda(select_top5_vector_only),
        )
        | property_finder_prompt | llm | StrOutputParser()
    )

sql_or_vector_branch = RunnableBranch(
    (has_sql_data, property_answer_from_sql_only()),
    property_answer_from_vector_only()   # default (SQL kosong)
)

# --- Final: gabungkan semua branch untuk type 'Pencarian'
hybrid_chain = sql_stage | sql_or_vector_branch
...
classifier_branches = RunnableBranch(
    ...
    (lambda x: x["cls"] == "1", hybrid_chain),          # Pencarian: SQL-first branch
